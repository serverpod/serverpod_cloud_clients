...
void main() {
  ...
  group('Given authenticated', () {
    ...
    group('when executing launch with --non-interactive', () {
      late Future commandResult;
      setUp(() {
        commandResult = cli.run(['launch', '--non-interactive']);
      });
        ...
        await expectLater(
          ...
          throwsA(
            isA<UsageException>().having(
              (e) => e.message,
              'message',
              'The launch command is interactive and cannot run with --non-interactive.',
            ),
          ),
        );
      });
...
''').create();
      ...
      group('and custom passwords in config/passwords.yaml', () {
        ...
        group('when executing launch for a new project '
          ...
          test(
            ...
            () async {
              ...
              expect(output, contains('> [ ] Select all'));
              expect(
                output,
                contains(
        ...
        group('when executing launch for a new project '
            'and selecting all passwords', () {
          late Future commandResult;

          setUp(() async {
            logger.answerNextConfirmsWith([true]);
            logger.inlineTerminal = FakeTerminal()
              ..queueInput([...FakeTerminal.space, ...FakeTerminal.enter]);
