  ...
  await runZonedGuarded(
    () async {
      try {
        ...
        await _preExit(logger);
        await errorReporter.reportError(e, stackTrace);
        exit(e.exitCode);
...
Future<void> _main(
  ...
  final runner = CloudCliCommandRunner.create(
    ...
    onAnalyticsConsentResolved: (final consented) =>
        errorReporter.analyticsConsent = consented,
    onCommandResolved: (final commandPath) =>
        errorReporter.command = commandPath,
    onApiServerResolved: (final apiServerUrl) =>
        errorReporter.apiServerUrl = apiServerUrl,
  );
  try {
