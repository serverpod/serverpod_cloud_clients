...
void main() {
  ...
  group('SentryErrorReporter.reportError -', () {
    ...
    group('Given consent is not resolved', () {
      test('when reporting a reportable error '
        ...
        await reporter.reportError(StateError('bug'), StackTrace.current);
    ...
    group('Given consent is granted', () {
      test('when reporting a reportable error '
        ...
        await reporter.reportError(error, StackTrace.current);
      ...
      test('when reporting an ErrorExitException '
        ...
        await reporter.reportError(ErrorExitException(), StackTrace.current);
      ...
      test('when the capture never completes '
          'then reportError completes after the flush timeout', () async {
        final reporter = createReporter(
        ...
        await expectLater(
          reporter.reportError(StateError('bug'), StackTrace.current),
          completes,
        );
      ...
      test('when the capture throws '
          'then reportError completes normally', () async {
        final reporter = createReporter(
          captureOverride: (final error, final stackTrace) =>
