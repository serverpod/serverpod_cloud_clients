...
void main() {
  ...
  group('Given supported Dart SDK versions', () {
    test('when no version source holds a value '
      ...
      expect(
        DartSdkSelector.selectDartSdkVersion(
    ...
    test('when the supported versions carry patch numbers '
        'then their minor version is selected', () {
      expect(
        DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: ['3.12.7', '3.13.2'],
        ),
        '3.13',
      );
    });

    test('when a supported version cannot be parsed '
        'then it is left out of the selection', () {
      expect(
        DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: ['3.11', 'not-a-version'],
        ),
        '3.11',
      );
    });

    test('when no supported version can be parsed '
        'then FailureException states that the fault is not the project', () {
      expect(
        () => DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: ['not-a-version'],
        ),
        throwsA(
  ...
  group('Given a requested version', () {
    ...
    test('when it is a caret constraint '
        'then the highest supported version it allows is selected', () {
      expect(
        DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: supported,
          scloudVersion: '^3.11.0',
        ),
        '3.13',
      );
    });

    test('when it allows any version '
        'then the highest supported version is selected', () {
      expect(
        DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: supported,
          pubspecVersionConstraint: 'any',
        ),
        '3.13',
      );
    });

    test('when it is higher than every supported version '
        'then FailureException lists the available versions', () {
      expect(
        () => DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: supported,
          commandLineVersion: '3.20',
        ),
        throwsA(
          isA<FailureException>().having(
            (final e) => e.errors.join(),
  ...
  group('Given several version sources', () {
    ...
    test('when scloud.yaml holds a version '
        'then it takes precedence over the lower sources', () {
      expect(
        DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: supported,
          scloudVersion: '3.12',
    ...
    test('when a higher-precedence source holds a blank value '
        'then it is skipped', () {
      expect(
        DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: supported,
          commandLineVersion: '  ',
          scloudVersion: '',
          toolVersionsVersion: '3.11',
        ),
        '3.11',
      );
    });

    test('when only .tool-versions holds a version '
        'then its minor version is selected', () {
      expect(
        DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: supported,
  ...
  group('Given a lockfile constraint', () {
    ...
    test('when it cannot be parsed '
        'then FailureException names pubspec.lock', () {
      expect(
        () => DartSdkSelector.selectDartSdkVersion(
          supportedSdkMinorVersions: supported,
          commandLineVersion: '3.12',
          lockVersionConstraint: 'not-a-version',
        ),
        throwsA(
          isA<FailureException>().having(
            (final e) => e.errors.join(),
