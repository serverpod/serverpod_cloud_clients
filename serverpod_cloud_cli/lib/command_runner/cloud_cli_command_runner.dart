...
class CloudCliCommandRunner extends BetterCommandRunner {
  ...
  /// Determines the project directory based on the global configuration,
  /// files found near the current directory, or user input if enabled.
  ///
  /// Validates that the directory is a valid Serverpod server directory,
  /// and gives feedback to the user.
  ///
  /// Throws [ExitException] if no valid project directory could be determined.
  Directory determineProjectDirectory() {
    final projectDirectory = Directory(_selectProjectDirectory());
    ...
    logger.info('Using project directory `${projectDirectory.path}`');
    return projectDirectory;
  }

  String _selectProjectDirectory() {
    ...
    // TODO: prompt user for project directory if in interactive mode
