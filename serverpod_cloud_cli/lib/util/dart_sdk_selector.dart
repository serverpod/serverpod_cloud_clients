...
_SourcedConstraint? _firstConstraint(final List<(String?, String)> sources) {
...
_SourcedConstraint? _constraint(final String? value, final String source) {
  final trimmed = value?.trim();
  if (trimmed == null || trimmed.isEmpty) {
    return null;
  }

  final normalized = _bareMinorVersion.hasMatch(trimmed)
      ? '$trimmed.0'
      : trimmed;
  ensureValidVersionConstraint(normalized, sourceDescription: '(from $source)');
...
final _bareMinorVersion = RegExp(r'^\d+\.\d+$');
...
List<Version> _minorVersionsOf(final List<String> minorVersions) {
...
Version? _minorVersionOf(final String minorVersion) {
